...
<body>
    ...
    <div class="stats">
        <div class="stat-item">Records: <span id="totalRecords">0</span></div>
        <div class="stat-item">Distance: <span>{$totalDistanceKm} km</span></div>
        <div class="stat-item">First: <span id="firstTime">-</span></div>
        <div class="stat-item">Last: <span id="lastTime">-</span></div>
        <div class="stat-item rest">Rest stops: <span>{$restCount}</span></div>
        <div class="stat-item warning">Speed anomalies: <span>{$anomalyCount}</span></div>
    </div>
    <div class="legend">
        <div class="legend-item"><div class="legend-dot normal"></div> Normal</div>
        <div class="legend-item"><div class="legend-dot rest"></div> Rest stop</div>
        <div class="legend-item"><div class="legend-dot anomaly"></div> Speed anomaly (&gt;50 km/h)</div>
    </div>
    <div id="map"></div>
    ...
    <script>
        ...
        const polyline = L.polyline(routeCoords, { color: '#3498db', weight: 3, opacity: 0.7 }).addTo(map);
        map.fitBounds(polyline.getBounds(), { padding: [20, 20] });

        // Add markers for start and end
        if (points.length > 0) {
            L.marker([points[0].lat, points[0].lng], {
                icon: L.divIcon({ className: 'start-marker', html: '<div style="background:#27ae60;color:white;padding:4px 8px;border-radius:4px;font-size:12px;white-space:nowrap;">Start</div>' })
            }).addTo(map);

            L.marker([points[points.length - 1].lat, points[points.length - 1].lng], {
                icon: L.divIcon({ className: 'end-marker', html: '<div style="background:#9b59b6;color:white;padding:4px 8px;border-radius:4px;font-size:12px;white-space:nowrap;">End</div>' })
            }).addTo(map);
        }
        ...
        restStops.forEach(stop => {
            ...
            }).addTo(map).bindPopup('<b>Rest Stop</b><br>' + stop.time + '<br>' + stop.addr);
        });
        ...
        speedAnomalies.forEach(anomaly => {
            ...
            }).addTo(map).bindPopup('<b>Speed Anomaly</b><br>' + anomaly.time + '<br>' + anomaly.speed + ' km/h');
        });
        ...
        timeline.forEach((item, index) => {
            ...
            div.innerHTML = '<span class="timeline-status">' + statusIcon + '</span>' +
                ...
                '<span class="timeline-distance">' + item.distance + 'm</span>' +
            ...
            div.addEventListener('click', () => {
                ...
                }).addTo(map).bindPopup('<b>' + item.time + '</b><br>' + item.addr + '<br>Speed: ' + item.speed + ' km/h').openPopup();
