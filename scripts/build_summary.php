    ...
    <script>
        ...
        if (points.length > 0) {
            L.marker([points[0].lat, points[0].lng], {
                icon: L.divIcon({ className: 'start-marker', html: '<div style="background:#27ae60;color:white;padding:4px 8px;border-radius:4px;font-size:12px;white-space:nowrap;">起點</div>' })
            }).addTo(map);

            L.marker([points[points.length - 1].lat, points[points.length - 1].lng], {
                icon: L.divIcon({ className: 'end-marker', html: '<div style="background:#9b59b6;color:white;padding:4px 8px;border-radius:4px;font-size:12px;white-space:nowrap;">終點</div>' })
            }).addTo(map);
        }
